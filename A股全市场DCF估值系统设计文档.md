...
[3] 更新估值结果库
    └── 记录新估值结果，标记为增量更新
```
新的 upside_pct = (implied_price - 新股价) / 新股价
新的 prob_upside = P(模拟股价 > 新股价)  ← 需要保存原始分布或百分位
...
| FCFF 为负（基期或个别年份） | 正常执行估值，结果标记 `fcff_warning`，报告中提示风险 |
| FCFF 为负（模拟中个别路径） | 属正常现象，蒙特卡洛分布自然包含负值路径，无需特殊处理 |
| FCFF 持续为负（3 年均为负） | 不应到达此处（已在 5.1 剔除）；若因数据更新触发，标记 failed 并移出池 |
